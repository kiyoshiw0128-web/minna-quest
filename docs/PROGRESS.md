...
| 11 | メールで合言葉を取り戻す | **実装済み。APIキー未設定のため未稼働** |

## いまの状態

- テスト **745件**（core 484 / worker 213 / web 48）、typecheck クリーン、CI 緑
...
- **2026-09-06、新UIを本番反映済み。** PR #1をmainへ取り込み、`40f2568`を`pnpm run release`で公開。未適用マイグレーションなし。公開画面で新UIを確認済み（Worker version: `14b840f6-3503-4fe0-8d69-902d1a2f421c`）。メール復旧の送信APIキー未設定は別途残る
...
- **合言葉も復旧コードも平文で保存しない。** どちらもハッシュだけを持つ。
  「いまの合言葉をメールで送る」形にすると平文の控えが要る。一度その設計で
  作りかけて差し戻した（`0009_recovery_codes.sql` の経緯）
- **復旧メールは `RESEND_API_KEY` が未設定だと送られない。** 応答は成功のままで、
  サーバのログにだけ「送っていない」と残る。届かないときはまずログを見る
