...
- テスト **716件**（core 483 / worker 188 / web 45）、typecheck クリーン、CI 緑
...
通知（未決。ブラウザ通知はiPhoneがホーム画面追加を要求する。Discordやメールなら
その制約が無い）。次にキャラクターの絵（プロンプトは `docs/boss-art-prompts.md`、
画像は人が別で作る）。
