...
describe('冒険の物語と戦闘結果', () => {
  it('前日の戦闘結果を物語の後、次回行動選択の前に表示する', async () => {
    ...
    installFetchMock({
      ...
      'POST /api/battle': jsonResponse(200, { ok: true, data: {
    ...
    const call = vi.mocked(fetch).mock.calls.find(([url]) => url === '/api/battle');
    expect(JSON.parse(String(call?.[1]?.body))).toEqual({ dayNo: 2, automatic: true });
    expect(screen.queryByRole('button', { name: 'このプランで挑む' })).not.toBeInTheDocument();
  });
});

it('自動戦闘の敗北は依頼の再取得後も残り、再処理されない', async () => {
  ...
  installFetchMock({
    'GET /api/today': jsonResponse(200, { ok: true, data }),
    'GET /api/world': jsonResponse(200, { ok: true, data: { currentDay: 3, history: [] } }),
    'POST /api/battle': jsonResponse(200, { ok: true, data: { won: false, worldDefeated: false,
      report: { enemy, party: [hero], rewarded: false, log: { result: 'lose', turns: 8, events: [{ t: 'end', result: 'lose', turns: 8 }] } },
    } }),
  });
  render(<TodayScreen token="t" onUnauthorized={vi.fn()} />);
  ...
  expect(vi.mocked(fetch).mock.calls.filter(([url]) => url === '/api/battle')).toHaveLength(1);
