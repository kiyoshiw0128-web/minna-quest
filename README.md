...
戦闘・育成・日次投票・闘技場・ペット・装備まで実装済み。
メール復旧は実装済みですが、送信APIキー未設定のため未稼働です。
段階別の状況と残課題は [進捗表](docs/PROGRESS.md) を参照してください。
...
#
#    release は移行を「適用」せず「未適用が無いか確かめる」だけにしてある。
#    wrangler は移行の適用時に対話で確認を求めるので、まとめた手順の中では
#    止まってしまう。そして schema の変更は取り消しが利かないため、自動で
#    承認するのは筋が悪い。未適用が残っていれば release は大声で止まる。
#    その場合は下の移行コマンドを人が走らせてから、もう一度 release する。
#
#    `run deploy` と書いているのは、pnpm が `deploy` を自身の組み込み
#    コマンドとして横取りするため。`run` を省くと別物が動いて失敗する。
